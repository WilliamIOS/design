...
- (void)textViewChanged:(NSNotification *)notification {
    ...
    NSLog(@"%ld",(long)[self.limitLength integerValue]);
...
//        NSString *headText = [self.text substringToIndex:selection.location];   // 光标前的文本
        ...
        NSInteger restLength = [self.limitLength integerValue] - tailText.length;
        
        if (realLength > [self.limitLength integerValue]) {
